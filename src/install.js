...
class Install extends Command {
  ...
  parseOptions(argv) {
    ...
    options
      ...
      .describe(
        'compatible',
        'Only install packages/themes compatible with this Pulsar version'
      );
    options
      .alias('h', 'help')
    ...
    options
      .alias('b', 'branch')
      .string('branch')
      .describe('branch', 'Sets the tag or branch to install');
    options
      .alias('t', 'tag')
      .string('tag')
      .describe('tag', 'Sets the tag or branch to install');
    options
      .alias('q', 'quiet')
      .boolean('quiet')
  ...
  async installModule(options, pack, moduleURI) {
    console.log('installModule:', options.installGlobally, options, pack, moduleURI);
    ...
    fs.makeTreeSync(installDirectory);

    // Extract the tarball.
    await pacote.extract(moduleURI, installDirectory);
    ...
    console.log(
      'DESTINATION index.js is at',
      path.join(installDirectory, 'index.js'),
      'and is',
      fs.readFileSync(
        path.join(installDirectory, 'index.js')
      ).toString()
    ...
    let { name, path: installPath } = await arborist.reify();
    ...
    // return { installPath: res.path };

    // console.log(
    //   `does node_modules/test-module/package.json exist?`,
    //   fs.readFileSync(
    //     path.join(installOptions.cwd, 'node_modules', 'test-module', 'package.json')
    //   ).toString()
    // );
    //
    // console.log(
    //   'DESTINATION index.js is',
    //   fs.readFileSync(
    //     path.join(installOptions.cwd, 'node_modules', 'test-module', 'index.js')
    //   ).toString()
    // );

    // return new Promise((resolve, reject) => {
    //   console.log(
    //     'FORK WITH installArgs:',
    //     installArgs,
    //     'installOptions:',
    //     installOptions
    //   );
  ...
  async installViaArborist (options) {
    let arborist = new Arborist({
      path: options.cwd
    });

    // Write the `package.json` into a

    let idealTree = arborist.buildIdealTree({
  ...
  // Request package information from the package API for a given package
  // name.
  //
  // * packageName: The string name of the package to request.
  //
  // Returns a Promise that rejects with an appropriate error or resolves to
  // the response body.
  ...
  //
  // * metadata: The package metadata object with at least a name key. A
  //   version key is also supported. The version defaults to the latest if
  //   unspecified.
  // * options: The installation options object.
  //
  // Returns a Promise; it either rejects with an error, or resolves to an
  // object representing data from the installed `package.json`.
  async installRegisteredPackage(metadata, options) {
    console.log('installRegisteredPackage', metadata, options);
    ...
    try {
      ...
      commands.push(
        async () => await this.installModule(options, pack, tarball)
      );
      if (
        installGlobally &&
        (packageName.localeCompare(pack.name, 'en', { sensitivity: 'accent' }) !== 0)
      ) {
        // The package was renamed! Delete the old package folder.
        commands.push(
          async newPack => {
  ...
  // * packagePath: The local path of the package in the form
  //   "file:./packages/package-name"
  // * options: The installation options object.
  //
  // Returns a Promise that resolves to the object representing the installed
  // `package.json` or rejects with an error.
  ...
  //
  // Returns a Promise that rejects with an error or resolves without a value.
  ...
  async installDependencies(options) {
    ...
    const commands = [];
    commands.push(
      async () => void await this.installModules(options),
      async () => void await this.installPackageDependencies(options)
    );
  ...
  async checkNativeBuildTools() {
    ...
    // This is more like invoking `ppm rebuild` on a fixed test directory, so
    // we'll be creating a `Rebuild` instance later on.

    const nativeModuleTestPath = path.resolve(__dirname, '..', 'native-module');
    const buildArgs = ['--globalconfig', config.getGlobalConfigPath(), '--userconfig', config.getUserConfigPath(), 'build'];
    buildArgs.push(nativeModuleTestPath);
    let buildFlags = this.getNpmBuildFlags();
    ...
    let rebuildCommand = new Rebuild();

    // Delegate this task to the `rebuild` command, but silently so that we can
    // do our own (minimal) logging.
    let { install: _result } = await rebuildCommand.invoke({
      cwd: nativeModuleTestPath,
      silent: true
    });
  ...
  async run(options) {
    ...
    const installPackage = async (name) => {
      ...
      if (gitPackageInfo || name.startsWith('file://')) {
        console.log('Installing from Git');
        return await this.installGitPackage(
          name,
          options,
          options.argv.branch || options.argv.tag
        );
      }
      if (name === '.') {
        await this.installDependencies(options);
...
`.yellow
    ...
    try {
      ...
      installedPackagesInfo = installedPackagesInfo.filter((_, idx) => packageNames[idx] !== ".");
